...
public class Herbivorve extends ALifeForm{

	/**
	 * Default Constructor for a herbivore
	 * @param name the name of the bug
	 * @param type the type of bug
	 * @param map  the map it belongs to
	 */
	public Herbivorve(String name,String type, Map map) {
		super(name,type, map);
	}
	...
	public void update() {
		...
		if(getEnergy() > 0){
			...
			//however if the food is meat we want to ignore it
			...
			if (overFood() == true) {
				...
				if(food.getType().equals("Meat")){
					...
					//ad the new energy from the food
